...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    frameName = sdfPtr->Get<std::string>("frameName");
    ROS_INFO_STREAM("frameName: " << frameName);
    robot_namespace =  sdf->Get<std::string>("robotNamespace") +"/";
    ROS_INFO_STREAM("robotNamespace: " << robot_namespace);
    if (downSample < 1) {
        downSample = 1;
    }
    ...
    // Resolve tf prefix
    std::string prefix;
    rosNode->getParam(std::string("tf_prefix"), prefix);
    if (robot_namespace != "/") {
        prefix = robot_namespace;
    }
    boost::trim_right_if(prefix, boost::is_any_of("/"));
    frameName = tf::resolve(prefix, frameName);
